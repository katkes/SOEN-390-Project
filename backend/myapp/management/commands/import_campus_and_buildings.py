...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("campus_file", type=str, help="Path to the Campus Boundaries GeoJSON")
        parser.add_argument("buildings_file", type=str, help="Path to the Building Information GeoJSON")
        parser.add_argument("boundaries_file", type=str, help="Path to the Building Boundaries GeoJSON")
    ...
    def handle(self, *args, **options):
        ...
        if not os.path.exists(buildings_file):
            self.stderr.write(self.style.ERROR(f"Buildings file {buildings_file} not found!"))
            return

        if not os.path.exists(boundaries_file):
            self.stderr.write(self.style.ERROR(f"Boundaries file {boundaries_file} not found!"))
            return
