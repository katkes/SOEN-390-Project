  ...
  group('CalendarEventService', () {
    test('fetchCalendarEvents groups events by date correctly', () async {
      ...
      when(mockCalendarRepository.getEvents(
              calendarId: 'testCalendar', useCache: true))
          .thenAnswer((_) async => mockEvents);

      final result =
          await calendarEventService.fetchCalendarEvents('testCalendar');
      ...
      verify(mockCalendarRepository.getEvents(
              calendarId: 'testCalendar', useCache: true))
          .called(1);
    });

    test('fetchCalendarEvents handles events with missing start time',
        () async {
      final mockEvents = [
        gcal.Event()
          ..id = 'event1'
    ...
    test('fetchCalendarEvents throws exception when repository throws',
        () async {
      when(mockCalendarRepository.getEvents(
              calendarId: 'testCalendar', useCache: true))
    ...
    test('deleteEventFromCache calls repository method', () async {
      ...
      // Assert
      verify(mockCalendarRepository.removeEventFromCache('event1', 'calendar1'))
          .called(1);
    });

    test('deleteEventFromCache throws exception when repository throws',
        () async {
      when(mockCalendarRepository.removeEventFromCache('event1', 'calendar1'))
          .thenThrow(Exception('Repository error'));
