...
class _CampusMapState extends State<CampusMap> {
  ...
  Future<void> _loadBuildingBoundaries() async {
    try {
      String geojsonString = await rootBundle.loadString('assets/campus_buildings.geojson');
      Map<String, dynamic> geojsonData = jsonDecode(geojsonString);
      
      if (!geojsonData.containsKey('features')) return;
      ...
      for (var feature in features) {
        ...
        List<LatLng> polygonPoints = coordinates.map<LatLng>((point) => LatLng(point[1], point[0])).toList();
        
        String name = feature['properties']['name'] ?? 'Unknown';
        String address = feature['properties']['address'] ?? 'No address';
    ...
    for (int i = 0; i < polygon.length - 1; i++) {
      ...
      if ((p1.latitude > point.latitude) != (p2.latitude > point.latitude)) {
        double intersectLon = (p2.longitude - p1.longitude) * (point.latitude - p1.latitude) /
            ...
            p1.longitude;
        if (point.longitude < intersectLon) intersections++;
      }
    }
      ...
      body: Stack(
        children: [
          FlutterMap(
            ...
            options: MapOptions(
              initialCenter: LatLng(45.497856, -73.579588),
              initialZoom: 14.0,
          ...
          if (_selectedBuildingName.isNotEmpty)
            Positioned(
              bottom: 20,
              left: 20,
              right: 20,
              child: Container(
                ...
                child: Column(
                  ...
                  children: [
                    Text(_selectedBuildingName, style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                    Text(_selectedBuildingAddress, style: TextStyle(fontSize: 14)),
                  ],
                ),
              ),
            ),
